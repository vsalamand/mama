...
class List < ApplicationRecord
  ...
  def get_suggestions
    ...
    recipe_tops = (top_recipe_category_ids - banned_products - list_categories - saved_products).map{|id| {id: id, context: "recipe_top"}}.each_slice(2).to_a
    data << recipe_tops
  ...
  def get_healthy_score
    score = self.get_rated_items([1]).size / self.get_rated_items([1, 2, 3]).size.to_f
