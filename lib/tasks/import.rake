...
namespace :import do
  ...
  task catalog: :environment do
    ...
    catalog.each do |row|
      ...
      if product.nil?
        food = Food.match_food(item["clean_description"])
      ...
      StoreItemHistory.create(store_item: store_item, price_per_unit: store_item.price_per_unit, is_promo: store_item.is_promo, is_available: store_item.is_available, date: Date.today)
